...
*Note: Adapter Generation Feature is available on MFP Devkits and Mobile Foundation Service only. For MFP Devkits, to use the adapter generation feature, you need to have JDK installed on the machine where MFP is installed and **JAVA_HOME** environment varibale needs to be set to the installed JDK path.*
